...
export const parseBatch = (text: string) => ((input: string): typia.Primitive<Batch.T> => { const assert = (input: any): Batch.T => {
    const __is = (input: any): input is Batch.T => {
        const $io0 = (input: any): boolean => "string" === typeof input.id && "string" === typeof input.sku && "number" === typeof input.quantity && (null === input.eta || input.eta instanceof Date) && (Array.isArray(input.allocations) && input.allocations.every((elem: any) => "object" === typeof elem && null !== elem && $io1(elem)));
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is Batch.T => {
            ...
            const $ao0 = (input: any, _path: string, _exceptionable: boolean = true): boolean => ("string" === typeof input.id || $guard(_exceptionable, {
                ...
                expected: "number",
                value: input.quantity
            })) && (null === input.eta || input.eta instanceof Date || $guard(_exceptionable, {
                path: _path + ".eta",
                expected: "(Date | null)",
                value: input.eta
